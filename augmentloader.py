...
class AugmentLoader:
    ...
    Notes:
        - number of augmentations and batch size are used to calculate the number of original
        images used in a batch
        - if num_aug = 0, then this dataloader is the same as a PyTorch dataloader, with
        the number of original images equal to the batch size, and each image is transformed
        using transforms from object argument.
        - AugmentLoader first samples from the dataset num_img of images, then apply augmentation
...
class BalanceSampler():
    """Samples data so that each class has the same number of samples. Performs sampling
    by first sorting data then uniformly sampling from each class with replacement."""
...
class RandomSampler():
    """Samples data randomly. Initializes sample indices when instantiated.
    Indices are shuffled if shuffle option is True. Sampling is performed by popping off
    the first index each time."""
